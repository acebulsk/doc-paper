...
bcak_comp <- read.csv('data/composite/bc_ak_composite_flows.csv')
...
comp_out <- composite_sum |> 
  ...
  ungroup() |> 
  pivot_wider(id_cols = c(IP_ID, ann_mm), names_from = Month, values_from = comp_mm) |> 
  left_join(npctr_bsn |> st_drop_geometry()) |> 
  mutate(q_km3yr = (ann_mm / 1e6) * Are_km2) |> 
  mutate(is_composite = 'yes') |> 
  select(IP_ID, Jan:Shap_Ar, ann_mm, q_km3yr:is_composite) |> 
  rbind(allmod)

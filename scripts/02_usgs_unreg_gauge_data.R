...
saveRDS(sites, 'data/gauged/usgs/usgs_1981_2010_site_meta.rds')
...
q_final <- readRDS('data/gauged/usgs/usgs_1981_2010_raw_monthly_flows.rds')
...
write.csv(flowdepth, "data/gauged/usgs/usgs_1981_2010_monthly_flows_long_2022.csv", row.names = F)

flowdepth <- read.csv("data/gauged/usgs/usgs_1981_2010_monthly_flows_long_2022.csv")

flowdepth_wide <- flowdepth |> 
  tidyr::pivot_wider(c(site_no, area), names_from = month, values_from = runoff) |> 
  mutate(ann_mm = rowSums(across(Jan:Dec, na.rm = TRUE))) |> 
  select(ID = site_no, area, Jan:ann_mm)

write.csv(flowdepth_wide, "data/gauged/usgs/usgs_1981_2010_monthly_flows_wide_2022.csv", row.names = F)
